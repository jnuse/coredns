...
* **OPTIONS**
  ...
  All name matching types support the following options
     ...
     * `answer ip IP DOMAIN` - rewrite the IP address in A or AAAA records when the domain name matches.
     * `answer ip file PATH` - load domain to IP mappings from a standard hosts file.
...
#### Rewriting IP Addresses in A and AAAA Records

The `rewrite` plugin supports rewriting IP addresses in the response's A and AAAA records based on domain name matching. This feature is useful for scenarios such as:

- **NAT traversal**: Return external IPs for internal domain names
- **IP address migration**: Transparently migrate services to new IP addresses
- **Testing and development**: Override production IPs with test environment IPs
- **Split-horizon DNS**: Return different IPs based on the query path

**Key Features:**
- Matches on the **domain name** in the DNS response record
- Replaces the IP regardless of what the backend DNS server returned
- Preserves all other record fields (TTL, class, etc.)
- Supports both IPv4 (A records) and IPv6 (AAAA records)

The syntax for IP address rewriting:
...
Or load domain-to-IP mappings from a standard hosts file:

```
rewrite [continue|stop] {
    name [exact|prefix|suffix|substring|regex] STRING STRING
    answer ip file /path/to/hosts-file
}
```

The hosts file should use standard `/etc/hosts` format:
```
...
10.0.0.2 server2.example.com server2-alias.example.com

# IPv6 examples
2001:db8:1::1 ipv6server.example.com
2001:db8:1::2 ipv6api.example.com ipv6api-alias.example.com
```

##### Example: Single IP Address Rewrite

Replace the IP address for a specific domain:
...
. {
    rewrite stop {
        ...
        answer auto
        answer ip 203.0.113.10 server.example.com
    }
    forward . 8.8.8.8
}
```

When a client queries for `server.example.com`, regardless of what IP the backend DNS returns (e.g., `192.168.1.100`), CoreDNS will rewrite it to `203.0.113.10` in the response.

**Query Example:**
```
$ dig @coredns server.example.com A
...
##### Example: Multiple IP Address Rewrites from Hosts File

Create a hosts file `/etc/coredns/ip-rewrites.txt`:
```
# Internal to external IP mappings
203.0.113.10 api.internal.example.com
203.0.113.20 web.internal.example.com
203.0.113.30 db.internal.example.com database.internal.example.com

# IPv6 mappings
2001:db8:1::10 api-v6.internal.example.com
```

Then use it in the Corefile:
...
Now all queries for domains in the hosts file will return the specified IPs:
- `api.internal.example.com` → `203.0.113.10`
- `database.internal.example.com` → `203.0.113.30` (alias support)
- `api-v6.internal.example.com` → `2001:db8:1::10`

##### Example: IPv6 Address Rewrite

Rewrite IPv6 addresses in AAAA records:
...
##### Example: Combining with Name Rewrite

You can combine IP rewriting with name rewriting in a single rule:

```
rewrite stop {
    name regex (.+)\.internal\.example\.com\. {1}.public.example.com.
    answer name (.+)\.public\.example\.com\. {1}.internal.example.com.
    answer ip 203.0.113.50 api.internal.example.com
}
```

This will:
1. Rewrite `api.internal.example.com` to `api.public.example.com` in the query
2. Forward to backend DNS
3. Rewrite response names back to `.internal.example.com`
4. Replace the IP to `203.0.113.50` for `api.internal.example.com`

**Important Notes:**
- IP rewriting is based on **domain name matching**, not IP address matching
- The original IP address returned by the backend DNS is **always replaced** if the domain matches
- When using file-based mappings, the file is loaded once at startup
- Only A and AAAA record IPs in the answer section are rewritten
- All other record fields (TTL, class, header) are preserved unchanged
- For IPv4 to IPv6 migration, configure separate A and AAAA mappings
