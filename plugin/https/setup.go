...
func setupDNSClient(conf *httpsConfig) dnsClient {
	...
	clients := make([]*metricDNSClient, len(conf.toURLs))
	...
	if conf.maxFails > 0 {
		opts = append(opts, withLbMaxFails(conf.maxFails))
	}
	if conf.timeout > 0 {
		opts = append(opts, withLbRequestTimeout(conf.timeout))
	}
...
func parseMaxFails(c *caddy.Controller, conf *httpsConfig) error {
	args := c.RemainingArgs()
	if len(args) != 1 {
		return c.ArgErr()
	}
	...
	conf.maxFails = uint32(n)
	return nil
}

func parseTimeout(c *caddy.Controller, conf *httpsConfig) error {
	args := c.RemainingArgs()
	if len(args) != 1 {
		return c.ArgErr()
	}
	...
	if timeout <= 0 {
		return c.Errf("timeout must be positive")
	}
